...
describe("Provider", function () {
  ...
  describe("Events", () => {
    it("triggers a successful rpc:result event when a successful response is received", async () => {
      const emittedEvents = [];
      ...
      assert.strictEqual(emittedEvents[1].args.length, 3);

      const [payload, error, result] = emittedEvents[1].args;
      assert.deepStrictEqual(payload, _stubbedPayload);
      assert.deepStrictEqual(error, null);
      assert.deepStrictEqual(result, _stubbedSuccessResult(_stubbedPayload));
    });

    it("triggers a failed rpc:result event when a failed response is received", async () => {
      const emittedEvents = [];
      ...
      try {
        ...
        assert.deepStrictEqual(
          error.code,
          _stubbedFailedResult(payload).error.code
        );
        assert.deepStrictEqual(
          error.message,
          _stubbedFailedResult(payload).error.message
        );
        assert.deepStrictEqual(
          error.data,
          _stubbedFailedResult(payload).error.data
        );
    ...
    it("triggers a rpc:request event when a request is sent", async () => {
      const emittedEvents = [];
      ...
      assert.deepStrictEqual(emittedEvents[0].args[0], _stubbedPayload);
    });
  });
});
